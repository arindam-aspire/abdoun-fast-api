...
- **List Your Property** stepper: **Redux / local–first** until **Save as Draft** or **Submit** (or legacy empty create + per-step `PATCH` on a persisted `submission_id`)
- **Admin moderation** (list, detail, review)
- **S3 presigned uploads** (binary never goes through PATCH; only `file_name` + `url` in the stepper `payload` / `PATCH` data; use `draft_client_id` when no `submission_id` yet)
...
4. [User APIs — property submissions](#4-user-apis--property-submissions)
...
**New (Redux-first) flow — no API on “Add property” alone**

1. Keep the full stepper in **local / Redux** until the user **Save as Draft** or **Submit**.
2. **Save as Draft (no `submission_id` yet):** `POST /property-submissions` with `payload` + optional `current_step` → receive `submission_id` and stored `payload` (see [4.1](#41-create--save-as-draft)).
3. **Submit without a prior draft id:** `POST /property-submissions/submit` with `payload` + `confirm_submit: true` → receive `submission_id` + `property_id` in one step (see [4.2](#42-direct-create-and-submit)).
4. For files **before** a `submission_id` exists: `POST /uploads/presigned-url` with **`draft_client_id`** (client-generated UUID) + file fields → `PUT` to S3 → store `url` in local state; after the first **Save as Draft**, you can presign with **`submission_id`** instead.
5. **Continue an existing server draft:** `GET` / `PATCH` / `POST .../{submission_id}/submit` as before (per-step `PATCH` is still valid for persisted submissions).

**Legacy / backward compatible:** `POST /property-submissions` with an **empty body** still creates an empty draft row. Older integrations may still do that on entry; new UIs should prefer local state + **Save as Draft** with `payload` instead of calling empty create on “Add property”.

**After a `submission_id` exists (any path):**

- **PATCH** each step as needed (`step` + `action` + `data`). Payload is **deep-merged** per step; lists in `data` replace that list in the merged step (do not rely on appending without sending the full list if your UX needs a full list).
- For files: **presign → PUT to S3 → PATCH** (or merge into the next **Save as Draft** `payload`) with `{ "file_name", "url" }` only (see [section 7](#7-s3-uploads--presigned-url-flow)).
- **Submit** (existing draft): `POST /property-submissions/{submission_id}/submit` with `confirm_submit: true` → backend validates, writes **normalized** property + media + owners + features, sets submission to **`submitted`**, returns `property_id`.
- **Admin** reviews (optional): approve / changes_requested / reject.
- If **`changes_requested`**, the same user can **PATCH** again and **submit** again (status returns to **`submitted`**).
...
```mermaid
stateDiagram-v2
  [*] --> draft: POST (empty or payload) / load existing
...
## 4. User APIs — property submissions

All under: **`/api/v1/property-submissions`** (plus one extra path below for direct submit).
...
**Primary (new flow):** send the full stepper state when the user first saves a draft (no call on “Add property” alone):
...
- `payload` is **deep-merged** into server defaults, validated as a draft, and `step_completion` / `last_completed_step` are computed.
- `current_step` is optional (default `1`, range 1–8).
- Response `data` includes `submission_id`, `status`, step fields, and **`payload`** (the stored merged state).

**Backward compatible:** omit the body, or send `{}` / `{ "payload": null }` — the server creates an **empty** draft (defaults only; `payload` in the response is **null**). Legacy apps may still use this on entry; new apps should use local/Redux state until **Save as Draft** with `payload` instead.

### 4.2 Direct create and submit

**POST** `/api/v1/property-submissions/submit`

For the first server write to be the final submit (e.g. full form in Redux, then one submit):
...
Response `data`: `submission_id`, `property_id`, `status` (e.g. `submitted`). Same validation and normalized persistence as `POST /property-submissions/{id}/submit`; on failure, the API does not leave a half-written property (transactional rollback on the server side).
...
**Full form “Save as Draft” (no `step`)** — same endpoint; send the entire stepper `payload` from local/Redux and the active `current_step`:
...
- Do **not** send `step` or `data` in this mode (validation error if you mix them with `payload`).
- `action` must be **`save_draft`** when `payload` is present.
- The body is deep-merged with server defaults, draft-validated the same as `POST /property-submissions` with a `payload`, and `saved_step` in the response is the step name for the given `current_step` (1–8).
...
### 4.5 Submit (existing `submission_id`)
...
The server sets each step to **`true` only when that step’s data looks meaningfully filled**, aligned with product rules used for a trustworthy draft:

| Step | Marked `true` when |
|------|--------------------|
| `basic_information` | `listing_purpose` sale/rent, non-blank `title`, `category_id` and `type_id` set |
| `location` | `city_id`, `area_id`, and **non-blank `address`** (after trim) |
| `owner_information` | At least one owner: non-blank `full_name`, and **phone or email** (non-blank) |
| `property_details` | At least one non-empty field in the object |
| `pricing` | `price` parses to a number **> 0** |
| `amenities` | `feature_ids` is a **non-empty** list with at least one id |
| `media_documents` | At least one **url** in `images` / `videos` / `documents` rows, **or** non-blank `youtube_url` / `virtual_tour_url` |
| `review_submit` | All four acceptance flags are `true` |

`GET` and `PATCH` (including full-payload save) return **recomputed** `step_completion` and **`last_completed_step`** (max index of `true` steps) from the current `payload`, so the JSON stays consistent for other clients, reporting, and debugging even when the in-app stepper uses local state for UI ticks.

**Submit** still runs full server-side validation regardless of these flags.
...
- **Exactly one** of `submission_id` (persisted draft) or `draft_client_id` (client UUID before a submission row) is required.
- With **`submission_id`:** the draft must belong to the current user; uploads are **blocked** when status is **`approved`** or **`rejected`** (409).
- With **`draft_client_id`:** no `property_listing_submissions` row is read; the user must still be **authenticated**; objects are written under `drafts/property-submissions/{draft_client_id}/...`.
...
**Option A — saved draft (existing behavior):**
...
**Option B — before a `submission_id` exists (Redux-first):**

```json
{
  "draft_client_id": "<client-generated-uuid>",
...
|-------|----------|--------|
| `submission_id` | One of `submission_id` or `draft_client_id` | Must belong to current user when set |
| `draft_client_id` | One of `submission_id` or `draft_client_id` | No DB row; same S3 path pattern as a real id |
...
- **AWS S3 console**: object appears under `drafts/property-submissions/{submission_id or draft_client_id}/...`
...
### 9.1 New listing — Redux-first (recommended)

1. (Optional) Generate a **`draft_client_id`** (UUID) on the client; use it for `POST /uploads/presigned-url` while building the form locally.
2. **Save as Draft:** `POST /property-submissions` with full `payload` (and `current_step` if you want) → store `submission_id` (or continue without a draft and submit in one step).
3. **Either** keep editing with `PATCH` per step, **or** re-save a full `payload` via a later **Save as Draft** pattern as your product allows.
4. For files after `submission_id` exists: `POST /uploads/presigned-url` with **`submission_id`**, or keep using the same `draft_client_id` paths until you migrate URLs in payload.
5. **Submit:** `POST /property-submissions/submit` (no prior id) with full `payload` + `confirm_submit: true`, **or** `POST /property-submissions/{submission_id}/submit` for a persisted draft.
6. (Optional) Admin: `GET` list → `GET` detail → `POST` review.

### 9.2 New listing — legacy (empty create still supported)

1. `POST /property-submissions` (empty body) → store `submission_id`.
2. For each step, `PATCH ...` with merged `data` (or full step object as you prefer).
3. For each file: `POST /uploads/presigned-url` (with `submission_id`) → `PUT upload_url` → `PATCH` owner or media with `{ file_name, url }`.
4. `POST .../submit` with `{ "confirm_submit": true }` → store `property_id`.
5. (Optional) Admin: `GET` list → `GET` detail → `POST` review.

### 9.3 Resubmit after admin changes
...
- [ ] **New flow:** do **not** require an API on “Add property”; store `submission_id` after the first **Save as Draft** (`POST` with `payload`) or after direct **Submit**, or from `GET` when resuming. Refetch `GET` after cold start / deep link when you have an id.
...
- [ ] Use `action` consistently: `save`, `next`, `previous`, `save_draft` ([section 4.4](#44-patch-step-save--navigate)).
...
1. **Save as Draft** (`POST` with `payload`) or empty create (compat) → copy `submission_id` when you need a persisted id.
2. Presign (with `submission_id` and/or `draft_client_id` while building locally) → **PUT** `upload_url` with binary file.
3. PATCH owner (or media), **or** include `url` rows in the next `payload` for Save as Draft.
4. GET submission → confirm payload.
5. Submit (direct or `.../{id}/submit`) → 200 + `property_id`.
